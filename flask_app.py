...
# Verhindere doppelte Route-Registrierung
...
def register_routes():
        ...
        if request.method == "POST":
            ...
            if nutzer:
                ...
                try:
                    ...
                    session['buchung_datum'] = spieldatum
                    session['buchung_beginn'] = beginn
                    session['buchung_ende'] = ende
    ...
    @login_required
    def bbestätigt():
        buchungs_nr = session.get('buchungs_nr', '')
        if not buchungs_nr:
        ...
        return render_template(
            ...
            nutzer_nid=session.get('nutzer_nid', ''),
            nutzer_vorname=session.get('nutzer_vorname', ''),
            nutzer_nachname=session.get('nutzer_nachname', ''),
            nutzer_email=session.get('nutzer_email', ''),
            nutzer_geburtsdatum=session.get('nutzer_geburtsdatum', ''),
            buchung_anlage=session.get('buchung_anlage', ''),
            buchung_platz=session.get('buchung_platz', ''),
            ...
            buchung_zeitpunkt=session.get('buchung_zeitpunkt', '')
        )
...
# Registriere alle Routes beim ersten Import
register_routes()